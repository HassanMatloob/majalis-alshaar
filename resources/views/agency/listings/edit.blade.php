...
<form action="{{ route('agency.listing.update', ['id' => $property->id]) }}" method="post">
    @method('PUT')
    @csrf
        ...
        <div class="grid grid-cols-12 gap-8">
            <div class="col-span-8">
                ...
                <div class="rounded-lg px-10 py-5 bg-white my-7">
                    <label for="name" class="text-base font-semibold text-[#575757]">Agent</label>
                    <div class="w-full p-4 bg-[#F2F2F2]  h-[99px] mt-3">
                        <label for="agent" class="text-sm font-semibold text-[#575757]">Assign Agent</label>
                        <select name="agent" id="agent" class="w-full mt-2 text-[#575757] font-normal focus:outline-none focus:border-transparent placeholder-gray-400 bg-[#F2F2F2]" required>
...
<script>
  ...
  Dropzone.options.documentDropzone = {
    url: "{{ route('agency.listing.image.store') }}",
    maxFilesize: 10, // MB
    addRemoveLinks: true,
