...
<div class="flex justify-between items-center">
    ...
    <div>
       <a href="{{ route('agent.listing.add') }}" class="text-white bg-primary hover:bg-primaryHover px-8 py-4 rounded-lg text-base font-semibold">Add Property</a>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        var ajaxConfig = {
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('agent.listings.fetch', ['filter' => 'all']) }}",
                type: 'POST',
                data: {}
            },
            columns: [
        ...
        $('button').on('click', (e) => {
            ...
            if(buttonId == 'all' || buttonId == 'for_sale' || buttonId == 'for_rent' || buttonId == 'featured') {
                ...
                ajaxConfig.ajax = "{{ route('agent.listings.fetch', ['filter' => ':filter']) }}".replace(':filter', buttonId);
